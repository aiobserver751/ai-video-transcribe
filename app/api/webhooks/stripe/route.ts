...
try {
    ...
    logger.info('[StripeWebhook] Loaded Credit Allowances:', tierCreditAllowance);
} catch (error) {
    logger.error("[StripeWebhook] Failed to load credit allowances from environment variables:", error);
    ...
    // logger.warn('[StripeWebhook] Using default credit allowances due to error.');
...
export async function POST(req: Request) {
  ...
  if (!endpointSecret) {
    logger.error('[StripeWebhook] STRIPE_WEBHOOK_SECRET environment variable is not set.');
  ...
  try {
    ...
    logger.info(`[StripeWebhook] Webhook received: ${event.type}`);
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Unknown error";
    logger.error(`[StripeWebhook] Webhook signature verification failed: ${message}`);
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      logger.info('[StripeWebhook] Handling checkout.session.completed', session.id);
      ...
      if (!session?.metadata?.userId || !session.subscription || !session.invoice || !session.customer) {
        logger.error('[StripeWebhook] Missing required data (userId, subscription, invoice, customer) in checkout session', {
      ...
      try {
        ...
        if (typeof periodEndTs === 'number') {
            ...
            logger.warn(`[StripeWebhook] Timestamp missing on sub item ${stripeSubscriptionId}. Trying top-level...`);
            ...
            if (typeof periodEndTs === 'number') {
                 ...
                 logger.warn(`[StripeWebhook] Timestamp missing on top-level sub ${stripeSubscriptionId}. Trying invoice ${invoiceId}...`);
        ...
        logger.info(`[StripeWebhook] Using current_period_end from ${source}: ${currentPeriodEnd}`);

        // --- Update DB ---
        logger.info(`[StripeWebhook] Updating user ${userId} via subscription ${stripeSubscriptionId} to tier ${newTier}.`);
        ...
        if (!creditTxResult.success) {
            logger.error(`[StripeWebhook] Failed to apply credits for new subscription ${stripeSubscriptionId} for user ${userId}: ${creditTxResult.error}`);
            ...
            logger.info(`[StripeWebhook] Credits successfully applied for new subscription ${stripeSubscriptionId} for user ${userId}. New balance: ${creditTxResult.newBalance}`);
        }
        ...
        logger.info(`[StripeWebhook] User ${userId} updated successfully via checkout session.`);
          ...
          logger.error(`[StripeWebhook] Error handling checkout.session.completed for user ${userId}:`, message, { userId, newTier, stripeSubscriptionId, stripeCustomerId, stripePriceId, currentPeriodEndTs, currentPeriodEnd, invoiceId });
    ...
    case 'invoice.payment_succeeded': {
        ...
        logger.info('[StripeWebhook] Handling invoice.payment_succeeded', invoice.id);
        ...
        if (invoice.billing_reason === 'subscription_cycle') {
            ...
            try {
                ...
                if (typeof stripeSubscriptionId === 'string') {
                    logger.info(`[StripeWebhook] Found subscription ID ${stripeSubscriptionId} directly on invoice ${invoice.id}.`);
                ...
                // Path 2: Fallback using Customer ID to find the user and their subscription ID
                    ...
                    logger.warn(`[StripeWebhook] Subscription ID missing on invoice ${invoice.id}. Falling back to customer ID ${stripeCustomerId}.`);
                    ...
                    if (user && user.stripeSubscriptionId) {
                         logger.info(`[StripeWebhook] Found user ${user.id} and their subscription ID ${user.stripeSubscriptionId} via customer ID.`);
                        ...
                        logger.error(`[StripeWebhook] Failed fallback: User not found or missing subscription ID for customer ${stripeCustomerId}.`);
                ...
                if (!creditTxResult.success) {
                    logger.error(`[StripeWebhook] Failed to apply credits for subscription renewal ${userSubscriptionId} for user ${finalUserIdForUpdate}: ${creditTxResult.error}`);
                    // Log and potentially alert, but the subscription itself is renewed.
                } else {
                    logger.info(`[StripeWebhook] Credits successfully reset for subscription ${userSubscriptionId} for user ${finalUserIdForUpdate}. Tier: ${tierForCreditRefill}, New balance: ${creditTxResult.newBalance}`);
                }

                logger.info(`[StripeWebhook] PRE-DB UPDATE (invoice.payment_succeeded): UserID=${finalUserIdForUpdate}, SubID=${userSubscriptionId}, Setting stripeCurrentPeriodEnd=${currentPeriodEnd?.toISOString()}`);
                await db.update(users)
                   .set({
                       stripeCurrentPeriodEnd: currentPeriodEnd,
                ...
                logger.info(`[StripeWebhook] Subscription ${userSubscriptionId} renewed successfully. Tier confirmed as ${tierForCreditRefill}. Credits reset.`);

            } catch (error: unknown) {
                const message = error instanceof Error ? error.message : "Unknown DB error";
                // Log context, including which ID was attempted
                logger.error(`[StripeWebhook] Error handling invoice.payment_succeeded renewal for invoice ${invoice.id}:`, message, { triedSubscriptionId: stripeSubscriptionId, triedCustomerId: stripeCustomerId, determinedUserSubscriptionId: userSubscriptionId });
                 return new NextResponse(`Webhook DB Error: ${message}`, { status: 500 });
            }
        } else {
            // Log why we are skipping this event (not a subscription cycle renewal)
            logger.info(`[StripeWebhook] Ignoring invoice.payment_succeeded for invoice ${invoice.id}. Reason: billing_reason is ${invoice.billing_reason ?? 'not subscription_cycle'}.`);
        }
        break;
    }
    ...
    case 'customer.subscription.updated': {
      ...
      logger.info(`[StripeWebhook] Handling ${event.type} for subscription ${subscriptionId}`);
      ...
      try {
          ...
          if (!user) {
              ...
              logger.error(`[StripeWebhook] Webhook Error: User not found for subscription ID: ${subscriptionId}`);
          ...
          if (isEndedStatus) {
              logger.info(`[StripeWebhook] Subscription ${subscriptionId} ended (Status: ${subscription.status}, Event: ${event.type}). Downgrading user ${userId}.`);
              ...
              if (currentDbTier !== 'free' && currentDbCreditBalance > 0) {
                  ...
                  if (!clearPaidCreditsResult.success) {
                      logger.error(`[StripeWebhook] Failed to clear paid credits for user ${userId} on subscription end: ${clearPaidCreditsResult.error}`);
                  } else {
                      logger.info(`[StripeWebhook] Cleared ${currentDbCreditBalance} paid credits for user ${userId}.`);
                  }
              }
              ...
              if (freeInitialCredits > 0) {
                  ...
                  if (!grantFreeCreditsResult.success) {
                      logger.error(`[StripeWebhook] Failed to grant initial free credits to user ${userId} on subscription end: ${grantFreeCreditsResult.error}`);
                  } else {
                      logger.info(`[StripeWebhook] Granted ${freeInitialCredits} initial free credits to user ${userId}.`);
                  }
              } else {
                  ...
                  logger.info(`[StripeWebhook] No initial free credits to grant for user ${userId} (FREE_TIER_INITIAL_CREDITS is 0 or less).`);
              }
              ...
              logger.info(`[StripeWebhook] User ${userId} downgraded successfully to free tier for ended subscription ${subscriptionId}.`);
          ...
          // Scenario 2: Plan Change (via Portal/API) - Price ID differs, still active, not marked for cancellation
              ...
              logger.info(`[StripeWebhook] Subscription ${subscriptionId} plan change detected for user ${userId}. DB Price: ${currentDbPriceId}, Event Price: ${eventPriceId}`);
              ...
              if (!newTier) {
                  ...
                  logger.error(`[StripeWebhook] Unrecognized Price ID ${eventPriceId} during subscription update for user ${userId}. Cannot map to internal tier.`);
                  ...
                  await db.update(users)
                    ...
                    .where(eq(users.id, userId));
                  logger.warn(`[StripeWebhook] User ${userId} subscription ${subscriptionId} updated with unrecognized price ID ${eventPriceId}. Stripe fields updated, tier/credits unchanged.`);
                  ...
                  if (isUpgrade || isDowngrade) {
                      ...
                      logger.info(`[StripeWebhook] User ${userId} ${changeType} to ${newTier}. Setting tier and credits immediately.`);
                      ...
                      if (!creditTxResult.success) {
                          logger.error(`[StripeWebhook] Failed to apply credits for ${changeType} to ${newTier} for user ${userId}: ${creditTxResult.error}`);
                      } else {
                          logger.info(`[StripeWebhook] Credits successfully reset for ${changeType} to ${newTier} for user ${userId}. New balance: ${creditTxResult.newBalance}`);
                      }

                  } else if (newTier === currentDbTier) {
                      // Price ID changed but mapped to the same tier (e.g., monthly vs annual)
                      logger.info(`[StripeWebhook] User ${userId} subscription ${subscriptionId} price ID changed but remains on ${newTier} tier. Stripe fields updated.`);
                  ...
                  if (!eventPeriodEnd) {
                      logger.warn(`[StripeWebhook] Subscription item ${subscriptionId} event data missing current_period_end. Refetching...`);
                      const freshSubscription = await stripe.subscriptions.retrieve(subscriptionId);
                      logger.debug("[StripeWebhook] Refetched Subscription Object:", JSON.stringify(freshSubscription, null, 2)); 
                  ...
                  logger.info(`[StripeWebhook] PRE-DB UPDATE (customer.subscription.updated - Plan Change): UserID=${userId}, SubID=${subscriptionId}, Updates: ${JSON.stringify(userUpdates)}`);
                  await db.update(users).set(userUpdates).where(eq(users.id, userId));
                  logger.info(`[StripeWebhook] User ${userId} plan update processed for subscription ${subscriptionId}. New Tier: ${userUpdates.subscriptionTier ?? currentDbTier}`);
              }
               ...
               if (!periodEndForLog) {
                   try {
                       ...
                       periodEndForLog = typeof periodEndForLogTs === 'number' ? new Date(periodEndForLogTs * 1000) : null;
                        ...
                        logger.error(`[StripeWebhook] Failed to refetch subscription ${subscriptionId} for cancellation logging`, fetchErr);
                   }
               }
               logger.info(`[StripeWebhook] Subscription ${subscriptionId} for user ${userId} updated. Marked to cancel at period end: ${periodEndForLog}`);
               await db.update(users)
                   .set({
                       subscriptionCancelledAtPeriodEnd: true
                    })
                   .where(eq(users.id, userId));
               logger.info(`[StripeWebhook] User ${userId} marked for cancellation at period end.`);
          ...
          // Scenario 4: Reactivated (cancel_at_period_end removed)
              ...
              logger.info(`[StripeWebhook] Subscription ${subscriptionId} for user ${userId} updated. Reactivated.`);
              await db.update(users)
                  .set({
                      subscriptionCancelledAtPeriodEnd: false
                    })
                   .where(eq(users.id, userId));
              logger.info(`[StripeWebhook] User ${userId} cancellation flag reset.`);
          }
          // Log other statuses if necessary
          else {
                logger.info(`[StripeWebhook] Subscription ${subscriptionId} updated for user ${userId}. Status: ${subscription.status}. No specific action taken.`);
          }

      } catch (error: unknown) {
            const message = error instanceof Error ? error.message : "Unknown DB error";
            // Add subscription ID to context if available
            logger.error(`[StripeWebhook] Error handling ${event.type} for subscription ${subscriptionId}:`, message);
    ...
    default:
      logger.warn(`[StripeWebhook] Unhandled webhook event type: ${event.type}`);
  }
